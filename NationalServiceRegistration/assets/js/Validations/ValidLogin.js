...
var DBSTRING = JSON.parse(localStorage['DBSTRING']);
...
//            } else {
//                markValid($(this));
//                try { helperText($(this), $helpText, $message, event); } catch (ss) { }
//            }
//        } else {
//            if ($checkEmail !== null) {
//                markValid($(this));
...
var ValidateFormData = function ($form) {
    ...
    $.each(fields, function (i, field) {
        ...
        if (field.type === 'email') {
            ...
            if ($(field).val() === '') {
                noErrors = false; 
                markInvalid($(field), $(field).parents($inputWrapper).siblings($helperClass).attr('data-error'));
            } else if ($checkEmail === null) {
                noErrors = false; 
